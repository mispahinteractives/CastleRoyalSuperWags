...
var checkMraid = /* @__PURE__ */ __name((initGame) => {
    if (typeof mraid === "undefined") {
        console.warn("MRAID not found! Providing a mock MRAID object for development.");

        window.mraid = {
            // MRAID states
            state: "loading", // Simulates the initial "loading" state
            isViewable: true, // Simulates viewable state
            listeners: {}, // Stores event listeners

            // Mock methods
            addEventListener: function(event, callback) {
                console.log(`Mock MRAID: addEventListener(${event})`);
                if (!this.listeners[event]) {
                    this.listeners[event] = [];
                }
                this.listeners[event].push(callback);

                // Simulate "ready" event after a short delay for testing
                if (event === "ready" && this.state === "loading") {
                    setTimeout(() => {
                        this.state = "default";
            ...
            removeEventListener: function(event, callback) {
                console.log(`Mock MRAID: removeEventListener(${event})`);
                if (this.listeners[event]) {
            ...
            getState: function() {
                console.log(`Mock MRAID: getState() -> ${this.state}`);
                return this.state;
            },

            isViewable: function() {
                console.log(`Mock MRAID: isViewable() -> ${this.isViewable}`);
                return this.isViewable;
            },

            open: function(url) {
                console.log(`Mock MRAID: open(${url})`);
                window.open(url, "_blank");
            },

            expand: function() {
                console.log("Mock MRAID: expand()");
                this.state = "expanded";
                this._triggerEvent("stateChange");
            },

            close: function() {
                console.log("Mock MRAID: close()");
            ...
            getPlacementType: function() {
                console.log("Mock MRAID: getPlacementType() -> inline");
                return "inline"; // Simulate "inline" placement
            },

            setResizeProperties: function(properties) {
                console.log(`Mock MRAID: setResizeProperties(${JSON.stringify(properties)})`);
            },

            getResizeProperties: function() {
                console.log("Mock MRAID: getResizeProperties()");
                return {}; // Simulate empty resize properties
            },

            _triggerEvent: function(event) {
                console.log(`Mock MRAID: Triggering event "${event}"`);
                if (this.listeners[event]) {
        ...
        console.log("Mraid found! Initializing game with MRAID support.");
